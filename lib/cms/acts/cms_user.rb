      ...
      module MacroMethods
        ...
        module InstanceMethods
          ...
          def viewable_sections
            @viewable_sections ||= Cms::Section.includes(:groups).references(Cms::Group.table_name).where(["#{Cms::Group.table_name}.id  in (?)", cms_groups.collect(&:id)])
          end

          def permissions
            @permissions ||= Cms::Permission.includes(:groups).references(Cms::Group.table_name).where(["#{Cms::Group.table_name}.id  in (?)", cms_groups.collect(&:id)])
          end
